...
import { getTrackingEventsPrefillOptions } from "../graphql/queries/trackingEventsPrefillOptions";
import { userById } from "react/graphql/queries/userQueries";
import { mapDataForTable } from "./prefill-data-fields-map";
import { updatePrefillOptions } from "../graphql/mutations/prefillOptions";
import { Color } from "@material-ui/lab/Alert";

export const TrackingEventsPrefillOptionsList: FunctionComponent = () => {
  ...
  const [trackingEventsPrefillOptions, setTrackingEventsPrefillOptions] =
    useState([]);
  ...
  const getFields = async (client, orgId) => {
    setLoading(true);
    const data = await getTrackingEventsPrefillOptions(
      client,
      {
        orgId,
      },
    ...
    setLoading(false);
    setTrackingEventsPrefillOptions(mapDataForTable(data));
  };
  ...
  async function onRowDelete(selectedRow) {
    const tracking_events = trackingEventsPrefillOptions.filter((option) => {
      if (option.id !== selectedRow.tableData.id) return option;
    });
    const { errors } = await updatePrefillOptions({
      client,
      variables: {
        input: { orgId, tracking_events },
      },
    });
    if (errors) {
      setSeverity("error");
  ...
  async function onRowAdd(row) {
    let max = 0;
    trackingEventsPrefillOptions?.forEach((option) => {
      if (max < option.id) {
        max = option.id;
      }
    });
    const tracking_events = [
      ...trackingEventsPrefillOptions,
      { id: max + 1, ...row },
    ];
    const { errors } = await updatePrefillOptions({
      client,
    ...
    <>
      ...
      >
        <Card className={classes.card}>
          <h3>
            New and deleted entries do not change existing tracking events.
          </h3>
          <h3>
            Changes here are available immediately in the Tracking Events
            Prefill list of a Tracking Event description for new and edited
            events.
          </h3>
          {isLoading ? (
            <LoadingSkeleton />
            ...
            <>
              <MaterialTable
                title={"Tracking Event Prefill List Entries"}
                columns={columns}
                data={trackingEventsPrefillOptions}
                isLoading={isLoading}
                options={{
                  search: false,
